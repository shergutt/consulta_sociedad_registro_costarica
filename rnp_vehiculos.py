#!/usr/bin/env python3
"""
Consulta de Vehiculos por Placa, Chasis, Motor, VIN o Serie en el Registro Nacional.

Uso:
    python3 rnp_vehiculos.py ABC123
    python3 rnp_vehiculos.py --chasis 1HGBH41JXMN109186
    python3 rnp_vehiculos.py --motor 12345678
    python3 rnp_vehiculos.py --vin 1HGBH41JXMN109186
    python3 rnp_vehiculos.py ABC123 --salida vehiculos
    python3 rnp_vehiculos.py ABC123 --no-guardar
    RNP_USER=correo@x.com RNP_PASS=MiClave python3 rnp_vehiculos.py ABC123
...
FORM_PATH = 'consultaDocumentos/paramConsultaVehiculo.jspx'
...
class RNP:
    ...
    def open_vehiculo_form(self):
        index_url = BASE + INDEX_PATH
        _, h = self._req(index_url, ref=BASE + 'login.jspx')
        vs = self._viewstate(h)
        link = find_vehiculo_link(h)
        if not vs or not link:
            raise SystemExit('✗ No se encontró la opción de Consulta de Vehículos.')
        data = {
            link['form']: link['form'],
            'javax.faces.ViewState': vs,
        }
        ...
        if 'Consulta de Vehículo' not in clean(form):
            raise SystemExit('✗ No se pudo abrir el formulario de consulta de vehículos.')
        return form

    def consulta(self, tipo_busqueda, valor):
        form = self.open_vehiculo_form()
        vs = self._viewstate(form)
        argus = extract_argus(form)
        url = BASE + FORM_PATH
        
        tipo_nombre = {
            'PLA': 'Consulta de Vehículo por Placa',
            'CHA': 'Consulta de Vehículo por Chasis',
            'MOT': 'Consulta de Vehículo por Motor',
            'VIN': 'Consulta de Vehículo por VIN',
            'SER': 'Consulta de Vehículo por Serie',
        }
        ...
        data = {
            ...
            'params:argus': argus,
            'params:j_id310': 'params:j_id310',
            'numeroConsulta': '26',
            'nombreConsulta': tipo_nombre.get(tipo_busqueda, 'Consulta de Vehículo'),
            'error': '',
            'javax.faces.ViewState': vs,
        }
...
def find_vehiculo_link(h):
    ...
    for m in token_re.finditer(body):
        ...
        if not text or not jsf:
            continue
        if current_section == 'Bienes Muebles' and text == 'Consulta de Vehículo':
            return {
...
def extract_argus(h):
    m = re.search(r'name="params:argus"[^>]*value="([^"]*)"', h)
    return m.group(1) if m else ''


def parse_vehiculos(h):
    vehiculos = []
    ...
    for idx, tr in enumerate(re.findall(r'<tr\b[^>]*>(.*?)</tr>', tbody.group(1), re.S), start=1):
        ...
        placa = clean(cells[0])
        marca = clean(cells[1])
        modelo = clean(cells[2])
        year = clean(cells[3]) if len(cells) > 3 else ''
        color = clean(cells[4]) if len(cells) > 4 else ''
        link = re.search(
...
def parse_result(h):
    ...
    notices = []
    for phrase in ('No hay registro', 'NO HAY VEHICULOS', 'NO HAY RESULTADOS'):
        if phrase.casefold() in text.casefold():
            notices.append(phrase)
    return {
        'notices': notices,
        'tables': tables,
        'vehiculos': parse_vehiculos(h),
        'text': text,
    }
...
def parse_vehiculo_detail(h):
    ...
    data = {
        ...
        'chasis': labels.get('CHASIS', ''),
        'combustible': labels.get('COMBUSTIBLE', ''),
        'cilindros': labels.get('CILINDROS', ''),
...
def safe_name(value):
    ...
    return value.strip('_') or 'vehiculo'


def default_output_dir(valor):
    return f'VEHICULO_{safe_name(valor).upper()}'


def save_vehiculo_files(out_dir, vehiculo, detail, html_body):
    os.makedirs(out_dir, exist_ok=True)
    base = safe_name(f'{vehiculo["index"]:02d}_{vehiculo["placa"]}')
    json_path = os.path.join(out_dir, base + '.json')
    txt_path = os.path.join(out_dir, base + '.txt')
    html_path = os.path.join(out_dir, base + '.html')

    payload = {'resumen': vehiculo, 'detalle': detail}
    with open(json_path, 'w', encoding='utf-8') as f:
        json.dump(payload, f, ensure_ascii=False, indent=2)
    with open(txt_path, 'w', encoding='utf-8') as f:
        f.write(render_vehiculo_text(vehiculo, detail))
    with open(html_path, 'w', encoding='utf-8') as f:
        f.write(html_body)
    return json_path, txt_path, html_path


def render_vehiculo_text(vehiculo, detail):
    lines = [
        f'Placa: {vehiculo["placa"]}',
        f'Marca: {vehiculo["marca"]}',
        f'Modelo: {vehiculo["modelo"]}',
        f'Año: {vehiculo["year"] or "-"}',
        f'Color: {vehiculo["color"] or "-"}',
        '',
    ]
    for key in ('placa', 'tipo', 'marca', 'modelo', 'year', 'color', 'serie',
                'motor', 'chasis', 'combustible', 'cilindros', 'propietario',
                'anotaciones', 'gravamenes'):
        if detail.get(key):
...
def main():
    ap = argparse.ArgumentParser(
        description='Consulta de Vehículos por Placa, Chasis, Motor, VIN o Serie en el RNP.')
    ap.add_argument('placa', nargs='?', help='Número de placa del vehículo')
    ap.add_argument('--chasis', help='Número de chasis')
    ap.add_argument('--motor', help='Número de motor')
    ap.add_argument('--vin', help='Número VIN')
    ...
    if not valor:
        raise SystemExit('✗ Indique una placa, --chasis, --motor, --vin o --serie.')
    ...
    print(f'• Consultando vehículo por {tipo_busqueda}: {valor}…', file=sys.stderr)
    result, result_html = rnp.consulta(TIPOS_BUSQUEDA[tipo_busqueda], valor)
    print_result(result)

    if not args.no_guardar and result['vehiculos']:
        out_dir = args.salida or default_output_dir(valor)
        print()
        print(f'Guardando detalles de {len(result["vehiculos"])} vehículo(s) en {out_dir}...')
        for vehiculo in result['vehiculos']:
            print(f'• Abriendo vehículo {vehiculo["placa"]}...', file=sys.stderr)
            detail, detail_html = rnp.vehiculo_detalle(result_html, vehiculo)
            json_path, txt_path, html_path = save_vehiculo_files(out_dir, vehiculo, detail, detail_html)
            print(f'- {vehiculo["placa"]}: {txt_path} | {json_path} | {html_path}')
