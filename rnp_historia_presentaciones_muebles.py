#!/usr/bin/env python3
"""
Consulta gratuita: Bienes Muebles -> Historia de Presentaciones.

Uso:
    python3 rnp_historia_presentaciones_muebles.py 2017-00240448
    python3 rnp_historia_presentaciones_muebles.py --desde-carpeta MARIA_203170516
"""
import argparse
import json
import os
...
def find_submit_params(h):
    match = re.search(
        r"jsfcljs\(document\.forms\['params'\],'([^']*numeroConsulta,56[^']*)'",
        h,
        re.S,
    )
    if not match:
        raise SystemExit('✗ No encontré el botón Consultar de historia de presentaciones muebles.')
    return parse_jsf_params(match.group(1))


def consulta_historia_presentacion(rnp, record):
    url, h = open_free_query(rnp, 'Bienes Muebles', 'Historia de Presentaciones')
    data = {
        'params': 'params',
        'tomo': record['tomo'],
...
def main():
    ap = argparse.ArgumentParser(description='Consulta historia de presentaciones de bienes muebles en RNP.')
    ...
    out_dir = args.salida or (
        f'{args.desde_carpeta}/historia_presentaciones_muebles' if args.desde_carpeta else 'historia_presentaciones_muebles')
    rnp = login_from_args(args)

    records = records_from_args(args)
    if not records:
        print('No se encontraron presentaciones de bienes muebles para consultar historia.')
        return

    for record in limit_records(records, args.limite):
        print(f'• Consultando historia presentación mueble {record["presentacion"]}…', file=sys.stderr)
        h = consulta_historia_presentacion(rnp, record)
        txt, js, raw = save_outputs(out_dir, record['presentacion'], h, {
            'consulta': 'Bienes Muebles - Historia de Presentaciones',
            'entrada': record,
        })
