#!/usr/bin/env python3
"""Ingest RNP analysis folder into PostgreSQL v2.

Schema: persons are shared across users, fincas/movable_assets belong to
persons (not to runs). The same cedula re-run updates existing fincas
in-place via UPSERT and creates a fresh query_run for audit history.
"""
import argparse
import hashlib
import json
...
def output_counts(folder, finca_records=None, movable_records=None):
    counts = {"finca_detalle": len(finca_records) if finca_records is not None else len(load_fincas(folder))}
    counts["bienes_muebles"] = len(movable_records) if movable_records is not None else len(load_movable_assets(folder))
...
def ingest_folder(db_url, folder_path, cedula=None, user_id=None):
    ...
    counts = output_counts(folder, finca_records, movable_records)
    ...
    try:
        # 1) Upsert person (shared by cedula)
        cur.execute("""
        ...
        # 2) Upsert query_run (UNIQUE on person_id+folder_path) for THIS folder;
        #    cross-folder runs become separate rows (audit history).
        cur.execute("""
            INSERT INTO query_runs (person_id, triggered_by_user_id, folder_path,
                report_path, report_markdown, finca_count, alert_count,
                output_counts_json, ran_at, created_at)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            ON CONFLICT (person_id, folder_path) DO UPDATE SET
                triggered_by_user_id = EXCLUDED.triggered_by_user_id,
                report_path = EXCLUDED.report_path,
                report_markdown = EXCLUDED.report_markdown,
                finca_count = EXCLUDED.finca_count,
                alert_count = EXCLUDED.alert_count,
                output_counts_json = EXCLUDED.output_counts_json,
                ran_at = EXCLUDED.ran_at
            RETURNING id
        """, (person_id, user_id, str(folder),
        ...
        cur.execute("DELETE FROM finca_query_outputs WHERE query_output_id IN (SELECT id FROM query_outputs WHERE query_run_id = %s)", (run_id,))
        cur.execute("DELETE FROM query_outputs WHERE query_run_id = %s", (run_id,))
        cur.execute("DELETE FROM alerts WHERE query_run_id = %s", (run_id,))
        cur.execute("DELETE FROM source_files WHERE query_run_id = %s", (run_id,))

        # 5) UPSERT fincas (shared by persona_id + numero + derecho + plano)
        #    and track which IDs survived so we can prune orphans below.
        finca_ids = []
        seen_natural = set()
        for order, record in enumerate(finca_records, 1):
            ...
            numero = str(resumen.get("numero") or detalle.get("finca") or "") or None
            derecho = str(resumen.get("derecho") or detalle.get("derecho") or "") or None
            plano = detalle.get("plano") or None
            natural = (numero, derecho, plano)
            seen_natural.add(natural)
            cur.execute("""
                INSERT INTO fincas (persona_id, query_run_id, index_no, provincia,
                ...
                ON CONFLICT (persona_id, numero, derecho, plano) DO UPDATE SET
                    ...
                    horizontal = EXCLUDED.horizontal,
                    matricula = EXCLUDED.matricula,
                    naturaleza = EXCLUDED.naturaleza,
                    ubicacion = EXCLUDED.ubicacion,
                    zona_catastrada = EXCLUDED.zona_catastrada,
                    medida = EXCLUDED.medida,
                    antecedentes = EXCLUDED.antecedentes,
                    identificador_predial = EXCLUDED.identificador_predial,
                    valor_fiscal_text = EXCLUDED.valor_fiscal_text,
                    valor_fiscal_num = EXCLUDED.valor_fiscal_num,
                    propietario = EXCLUDED.propietario,
                    anotaciones = EXCLUDED.anotaciones,
                    gravamenes = EXCLUDED.gravamenes,
                    resumen_json = EXCLUDED.resumen_json,
                    detalle_json = EXCLUDED.detalle_json,
                    texto = EXCLUDED.texto,
                    source_json = EXCLUDED.source_json,
                    source_txt = EXCLUDED.source_txt,
                    source_html = EXCLUDED.source_html,
                    updated_at = EXCLUDED.updated_at
                RETURNING id
            """, (person_id, run_id, index_no,
                  resumen.get("provincia") or detalle.get("provincia"),
                  province_code(resumen),
                  numero, derecho,
                  str(resumen.get("duplicado") or detalle.get("duplicado") or "") or None,
                  str(resumen.get("horizontal") or detalle.get("horizontal") or "") or None,
                  detalle.get("matricula"), detalle.get("naturaleza"),
                  detalle.get("ubicacion"), detalle.get("zona_catastrada"),
                  detalle.get("medida"), plano,
        ...
        # 6) Prune fincas whose natural key is no longer present in this run
        #    (only prune ones that pointed to THIS run; older runs' fincas stay).
        cur.execute("""
            DELETE FROM fincas
            WHERE persona_id = %s
              AND query_run_id = %s
              AND (numero, derecho, plano) IS NOT NULL
              AND (numero, COALESCE(derecho,''), COALESCE(plano,'')) NOT IN %s
        """, (person_id, run_id,
              tuple((n or '', d or '', p or '') for (n, d, p) in seen_natural) or (("", "", ""),)))

        # 7) UPSERT movable_assets (shared by persona + natural keys)
        asset_count = 0
        seen_asset_natural = set()
        for order, record in enumerate(movable_records, 1):
            ...
            placa = detalle.get("placa") or None
            vin = detalle.get("vin") or None
            serie = detalle.get("serie") or None
            motor = detalle.get("motor") or None
            chasis = detalle.get("chasis") or None
            seen_asset_natural.add((placa, vin, serie, motor, chasis))
            cur.execute("""
                INSERT INTO movable_assets (persona_id, query_run_id, index_no, asset_type,
                    identificacion, nombre, tipo, numero, placa, matricula, marca, modelo,
                    year, color, serie, vin, motor, chasis, propietario, cedula_propietario,
                    estado, anotaciones, gravamenes, resumen_json, detalle_json, texto,
                    source_json, source_txt, source_html, created_at, updated_at)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s,
                        %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                ON CONFLICT (persona_id, placa, vin, serie, motor, chasis) DO UPDATE SET
                    query_run_id = EXCLUDED.query_run_id,
                    index_no = EXCLUDED.index_no,
                    ...
                    propietario = EXCLUDED.propietario,
                    cedula_propietario = EXCLUDED.cedula_propietario,
                    ...
                    updated_at = EXCLUDED.updated_at
                RETURNING id
            """, (person_id, run_id, resumen.get("index") or order, "bien_mueble",
        ...
        # Prune orphan movable_assets that pointed to this run and no longer appear
        if seen_asset_natural:
            cur.execute("""
                DELETE FROM movable_assets
                WHERE persona_id = %s
                  AND query_run_id = %s
                  AND (placa, vin, serie, motor, chasis) NOT IN %s
            """, (person_id, run_id, tuple(seen_asset_natural) or (("",)*5,)))

        # 8) Insert query_outputs (per-run, not shared)
        output_count = 0
        for subdir, query_type in QUERY_DIRS.items():
            target = folder / subdir
            if not target.exists():
            ...
            for json_path in sorted(target.glob("*.json")):
                ...
                cur.execute("""
                    INSERT INTO query_outputs (query_run_id, query_type, lookup_key, consulta,
        ...
        for path in sorted(folder.rglob("*")):
            ...
            try:
                ...
                continue
            cur.execute("""INSERT INTO source_files (query_run_id, relative_path, file_type,
                size_bytes, sha256, content_text, created_at)
                VALUES (%s, %s, %s, %s, %s, %s, %s)
                ON CONFLICT (query_run_id, relative_path) DO UPDATE SET
        ...
        for idx, (record, label, severity, message, source) in enumerate(flags):
            ...
            cur.execute("""INSERT INTO alerts (query_run_id, finca_id, severity, label, message, source, created_at)
                VALUES (%s, %s, %s, %s, %s, %s, %s)""",
                (run_id, finca_pk, severity, label, message, source, timestamp))
        inserted_alerts = len(flags)

        if inserted_alerts != alert_count:
            cur.execute("UPDATE query_runs SET alert_count = %s WHERE id = %s",
                        (inserted_alerts, run_id))
